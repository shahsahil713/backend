...
export class UserManager {
    ...
    public queryUsers = async (filter?: FilterQuery<User>, options?: ProjectionType<User>): Promise<User[]> => {
        const users = await User.paginate(filter, options);
        return users;
    };

    public updateUserById = async (userId: ObjectId | string, updateBody: User): Promise<User> => {
        const user = await this.getUserById(userId);
        if (!user) {
            throw new ApiError(httpStatus.NOT_FOUND, 'User not found');
        }
        if (updateBody.email && (await User.isEmailTaken(updateBody.email, String(userId)))) {
            throw new ApiError(httpStatus.BAD_REQUEST, 'Email already taken');
        }
